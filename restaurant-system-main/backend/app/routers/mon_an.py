...
def update_dish_endpoint(
    ...
    old_price = dish.gia_ban
    dish = update_dish(db, dish_id, payload)
    if payload.gia_ban is not None and payload.gia_ban != old_price:
        db.add(
            NhatKyThaoTac(
                nhan_vien_id=current_user.id,
                vai_tro=current_user.vai_tro,
                tai_khoan=current_user.ten_dang_nhap,
                hanh_dong="SUA_GIA_MON",
                doi_tuong="MON_AN",
                doi_tuong_id=dish.id,
                du_lieu_cu={"gia_ban": old_price},
                du_lieu_moi={"gia_ban": payload.gia_ban},
