...
def create_dish(db: Session, payload: MonAnCreate) -> MonAn:
    ...
    dish = MonAn(
        ...
        don_vi_tinh=payload.don_vi_tinh.strip(),
        mo_ta=payload.mo_ta.strip() if payload.mo_ta else None,
        thoi_gian_che_bien_phut=payload.thoi_gian_che_bien_phut,
...
def update_dish(db: Session, dish_id: int, payload: MonAnUpdate) -> MonAn:
    ...
    if payload.gia_ban is not None:
        dish.gia_ban = payload.gia_ban
    if payload.don_vi_tinh is not None:
        dish.don_vi_tinh = payload.don_vi_tinh.strip()
    if payload.mo_ta is not None:
        dish.mo_ta = payload.mo_ta.strip() or None
    if payload.thoi_gian_che_bien_phut is not None:
        dish.thoi_gian_che_bien_phut = payload.thoi_gian_che_bien_phut
