...
export default function DishList({ dishes, loading, categories, onEdit, onDelete }) {
  ...
  return <div className="menu-category-list">
    {dishes.map((dish) => <article className="category-row dish-row" key={dish.id}>
      ...
      <div className="category-main"><strong>{dish.ten_mon}</strong><span>{dish.nhom_mon_ten || categoryName(dish.nhom_mon_id)} · {formatVnd(dish.gia_ban)} · {dish.don_vi_tinh || '—'} · {dish.thoi_gian_che_bien_phut || '—'} phút</span></div>
