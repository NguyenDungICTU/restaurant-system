...
export default function AuditLogs() {
  ...
  const actionColumns = [
    ...
    { title: 'Món / đối tượng', render: (_, row) => row.object_id ? `${row.object_type} #${row.object_id}` : row.object_type },
  ...
  return <section className="page-content">
    <div className="page-heading">
      <div><p className="eyebrow">SECURITY & AUDIT</p><h1>Nhật ký hệ thống</h1><p className="subheading">Tra cứu ai đăng nhập và ai đã thay đổi giá niêm yết.</p></div>
      <Button icon={<ReloadOutlined />} loading={loading} onClick={load}>Làm mới</Button>
    </div>
    {error && <Alert type="error" message={error} showIcon />}
    <div className="panel" style={{ marginTop: 24 }}>
      <Tabs items={[
        { key: 'actions', label: 'Thay đổi giá & thao tác', children: <><Select value={filter} onChange={setFilter} style={{ width: 220, marginBottom: 16 }} options={[{ value: 'all', label: 'Tất cả thao tác' }, { value: 'CAP_NHAT_GIA_MON', label: 'Chỉ thay đổi giá' }, { value: 'DANG_NHAP', label: 'Đăng nhập' }, { value: 'DANG_XUAT', label: 'Đăng xuất' }]} /><Table rowKey="id" loading={loading} columns={actionColumns} dataSource={actions} pagination={{ pageSize: 10 }} /></> },
        { key: 'sessions', label: 'Phiên đăng nhập', children: <Table rowKey="id" loading={loading} columns={sessionColumns} dataSource={sessions} pagination={{ pageSize: 10 }} /> },
      ]} />
    </div>
  </section>
}
